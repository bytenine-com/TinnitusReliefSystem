...
package com.bytenine.tinnitusrelief.service.contentcatalogs;
...
import com.bytenine.tinnitusrelief.BuildConfig;
...
public class MusicLibrary {
    ...
    public static List<MediaBrowserCompat.MediaItem> getMediaItems(Context context) {
        List<MediaBrowserCompat.MediaItem> result = new ArrayList<>();
        Log.e("*******", "music.size: " + music.size());
/*        if (null != context) {
            Log.e("*******", "context not null!!!!");
            preferences = context.getSharedPreferences("TRS", 0);
        }
        if (MainActivity.favIsPressed) {
            MusicLibrary.getMediaItems(context).clear();*/
            for (MediaMetadataCompat metadata : music.values()) {


                  /* Log.e("******************","MediaBrowserCompat title: " +  metadata.getDescription().getTitle());
                String title = metadata.getDescription().getTitle() + ".mp3";
                boolean isSongFav = preferences.getBoolean(title, false);

                if (isSongFav) {
                    Log.e("******************", "Song is Favourite " + title);*/
                result.add(
                        new MediaBrowserCompat.MediaItem(
                                metadata.getDescription(), MediaBrowserCompat.MediaItem.FLAG_PLAYABLE));
              /*  }
                  else Log.e("******************","Song is NOT Favourite " +title);
    ...
    public static void clearMusicList () {
        Log.e("*******", "music.size before: " + music.size());
        music.clear();
        albumRes.clear();
        musicFileName.clear();
        Log.e("*******", "music.size after: " + music.size());
